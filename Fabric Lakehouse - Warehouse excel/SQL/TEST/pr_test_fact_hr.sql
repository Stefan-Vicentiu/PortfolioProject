...
CREATE OR ALTER PROCEDURE [test_data].[pr_test_fact_hr]
AS
BEGIN
...
BEGIN TRY
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM [test_data].[Hr] AS target 
        WHERE target.HR_ID = source.HR_ID
    )
        AND source.Processing_Date >= (SELECT ISNULL(MAX(Processing_Date), '1900-01-01') FROM [test_data].[Hr]);

    -- Count modified rows
    SELECT 
        @RowsUpdated = COUNT(*)
    FROM [test_data].[Hr] AS target
    ...
    -- Incremental Load
    MERGE INTO [test_data].[Hr] AS To_Target 
        USING (
            SELECT
                HR_ID,
                Date_ID,
            ...
            WHERE Processing_Date >= ( SELECT ISNULL(MAX(Processing_Date), '1900-01-01') FROM [test_data].[Hr] )
        ) AS From_Source
