...
CREATE OR ALTER PROCEDURE [test_data].[pr_test_dim_business_lines]
AS
BEGIN
...
BEGIN TRY
    ...
    BEGIN
        -- create BusinessLines dimension
        CREATE TABLE [test_data].[BusinessLines] (
    ...
    TRUNCATE TABLE [test_data].[BusinessLines];

    -- Insert all rows from Lakehouse
    INSERT INTO [test_data].[BusinessLines] (
        Business_Line_ID,
        Business_Line_Name,
        Region,
        Status
    ...
    SELECT 
        ...
        Region,
        Status
    FROM [Lakehouse_RawData].[dbo].[dim_business_lines];
