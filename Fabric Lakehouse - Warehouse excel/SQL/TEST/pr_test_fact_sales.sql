...
CREATE OR ALTER PROCEDURE [test_data].[pr_test_fact_sales]
AS
BEGIN
...
BEGIN TRY
    ...
    BEGIN
        -- create Sales fact in test_data with calculated columns
        CREATE TABLE [test_data].[Sales] (
            ...
            -- new columns from DAX logic
            Days_Outstanding INT,
            Delivery_Days INT
        );
    END

    -- get last transaction date for incremental logic from target
    DECLARE @LastTransactionDate DATETIME2(6) = (SELECT ISNULL(MAX(Transaction_DateTime), '1900-01-01 00:00:00.000000') FROM [test_data].[Sales]);

    -- Count new rows
    SELECT 
        @RowsInserted = COUNT(*)
    FROM [dev_data].[Sales] AS source
    WHERE NOT EXISTS (
        SELECT 1 FROM [test_data].[Sales] AS target 
        WHERE target.Sales_ID = source.Sales_ID
    )
        AND source.Transaction_DateTime >= @LastTransactionDate;

    -- Count modified rows
    SELECT 
        @RowsUpdated = COUNT(*)
    FROM [test_data].[Sales] AS target
    INNER JOIN [dev_data].[Sales] AS source
    ...
    -- incremental load using MERGE from dev_data to test_data
    MERGE INTO [test_data].[Sales] AS To_Target 
        USING (
            SELECT
                *,
                -- DAX: DATEDIFF(Transaction Date, TODAY(), DAY)
                DATEDIFF(DAY, CAST(Transaction_DateTime AS DATE), CAST(GETDATE() AS DATE)) AS Calc_Days_Outstanding,
                DATEDIFF(DAY, CAST(Transaction_DateTime AS DATE), CAST(GETDATE() AS DATE)) AS Calc_Delivery_Days
            FROM [dev_data].[Sales]
    ...
    THEN UPDATE SET 
        ...
        To_Target.Days_Outstanding = From_Source.Calc_Days_Outstanding,
        To_Target.Delivery_Days = From_Source.Calc_Delivery_Days
    ...
    WHEN NOT MATCHED BY TARGET THEN
        ...
        VALUES (
            ...
            From_Source.Calc_Days_Outstanding, 
            From_Source.Calc_Delivery_Days
        );
