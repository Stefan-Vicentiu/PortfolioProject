...
CREATE OR ALTER PROCEDURE [dev_data].[pr_dev_fact_inventory]
AS
BEGIN
...
BEGIN TRY
    ...
    BEGIN
        ...
        CREATE TABLE [dev_data].[Inventory] (
    ...
    DECLARE @TransactionDateTime DATETIME2(6) = (SELECT ISNULL(MAX(Transaction_DateTime), '1900-01-01 00:00:00.000000') FROM [dev_data].[Inventory]);
    ...
    WHERE 
        NOT EXISTS (
            SELECT 1 FROM [dev_data].[Inventory] AS To_Target 
    ...
    SELECT 
        @RowsUpdated = COUNT(*)
    FROM [dev_data].[Inventory] AS To_Target
    ...
    -- Incremental Load
    MERGE INTO [dev_data].[Inventory] AS To_Target 
        USING (
            SELECT
                Inventory_ID,
                Date_ID,
