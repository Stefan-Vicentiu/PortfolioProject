...
CREATE OR ALTER PROCEDURE [dev_data].[pr_dev_fact_budget]
AS
BEGIN
...
BEGIN TRY    
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM [dev_data].[Budget] AS target 
        WHERE target.Budget_ID = source.Budget_ID
    )
        AND source.Last_Modified_Date >= (SELECT ISNULL(MAX(Last_Modified_Date), '1900-01-01') FROM [dev_data].[Budget]);

    -- Count modified rows
    SELECT 
        @RowsUpdated = COUNT(*)
    FROM [dev_data].[Budget] AS target
    ...
    -- Incremental Load
    MERGE INTO [dev_data].[Budget] AS To_Target 
        USING (
            SELECT
                Budget_ID,
                Date_ID,
            ...
            WHERE Last_Modified_Date >= ( SELECT ISNULL(MAX(Last_Modified_Date), '1900-01-01') FROM [dev_data].[Budget] )
        ) AS From_Source
