...
CREATE OR ALTER PROCEDURE [dev_data].[pr_dev_fact_purchase]
AS
BEGIN
...
BEGIN TRY
    ...
    BEGIN
        ...
        CREATE TABLE [dev_data].[Purchases] (
    ...
    DECLARE @LastTransactionDate DATETIME2(6) = (SELECT ISNULL(MAX(Transaction_Date), '1900-01-01 00:00:00.000000') FROM [dev_data].[Purchases]);
    ...
    WHERE 
        NOT EXISTS (
            SELECT 1 FROM [dev_data].[Purchases] AS To_Target 
    ...
    SELECT 
        @RowsUpdated = COUNT(*)
    FROM [dev_data].[Purchases] AS To_Target
    ...
    -- Incremental Load
    MERGE INTO [dev_data].[Purchases] AS To_Target 
        USING (
            SELECT
                Purchase_ID,
