...
CREATE OR ALTER PROCEDURE [prod_data].[pr_prod_dim_business_lines]
AS
BEGIN
...
BEGIN TRY
    ...
    IF NOT EXISTS (
        ...
        WHERE t.name = 'BusinessLines' 
            AND s.name = 'prod_data'
    )
    BEGIN
        -- create BusinessLines dimension in prod_data
        CREATE TABLE [prod_data].[BusinessLines] (
            [Business Line Code] VARCHAR(255) NOT NULL,
            [Business Line Name] VARCHAR(255),
            Region VARCHAR(100),
            Status VARCHAR(50)
        );
    END
    ...
    TRUNCATE TABLE [prod_data].[BusinessLines];

    -- Insert all rows from test_data to prod_data
    INSERT INTO [prod_data].[BusinessLines] (
        [Business Line Code],
        [Business Line Name],
        Region,
        Status
